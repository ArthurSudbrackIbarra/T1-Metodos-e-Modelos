...
@given('The user uploads the Chevrolet PDF file located in "{pdf_path}"')
def step_given_upload_chevrolet_pdf(context, pdf_path):
    global global_pdf_path
    global_pdf_path = path.abspath(pdf_path)


@when('I send a POST request to /pdfs/upload with montadora set to "{montadora}"')
...
@then('the created pdf must have the same name as the uploaded file')
...
@then('the extracted sigla must be "{expected_sigla}"')
...
@then('the extracted desc_cat must be "{expected_desc_cat}"')
